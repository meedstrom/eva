...
* Important news
*** [2021-08-28 Sat]
1. Turns out if you typed "/help" in a query, it'd append a row to the dataset including the value "/help".  While not a breaking problem, you can now prevent it in your =eva-defun= bodies by checking that they got a non-nil input from =eva-read= or =eva-read-string=.  See updated queries in [[file:eva-builtin.el]].
...
2. New commits MAY break the package for days at a time and deprecations are frequent.
